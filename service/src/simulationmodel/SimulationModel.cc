...
IEntity* SimulationModel::createEntity(const JsonObject& entity) {
    ...
    // Use the composite factory to create the entity
    IEntity* myNewEntity = compositeFactory.CreateEntity(entity);


    if (!myNewEntity) {
        std::cout << "[!] Error: Unsupported entity type: " << entity["type"].toString() << std::endl;
        return nullptr;
    }
    ...
    std::cout << "Entity created, information: " << myNewEntity->getName() <<  myNewEntity->getPosition() << std::endl;
